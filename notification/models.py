...
class NoticeType(models.Model):
    ...
    categorie = models.CharField(_('categorie'), max_length=40)
...
class Notice(models.Model):
    ...
    #attach = models.ForeignKey(ContentType, null=True, blank=True)
...
def create_notice_type(label, display, description, categorie='', default=2, verbosity=1):
    """
    ...
    try:
        ...
        if default != notice_type.default:
            ...
            updated = True
        if updated:
            notice_type.categorie = categorie
            notice_type.save()
            if verbosity > 1:
                print "Updated %s NoticeType" % label
    except NoticeType.DoesNotExist:
        NoticeType(label=label, display=display, description=description, categorie=categorie, default=default).save()
        if verbosity > 1:
...
def send_now(users, label, extra_context=None, on_site=True, sender=None, attach=None):
    """
    ...
    You can link an object as an attachement with the sent notice:

    notification.send(user, "label", {
        "spam": "eggs",
        "foo": "bar",},
